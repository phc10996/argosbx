...
warpsx(){
warpurl=$( (command -v curl >/dev/null 2>&1 && curl -sm5 -k https://api.cloudflareclient.com（需自行配置WARP密钥） 2>/dev/null) || (command -v wget >/dev/null 2>&1 && timeout 3 wget --tries=2 -qO- https://__WARP_SOURCE__ 2>/dev/null) )
if [ -n "$warpurl" ]; then
